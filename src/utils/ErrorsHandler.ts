...
export class ErrorsHandler implements ErrorHandler {
  ...
  handleError(error: Error | HttpErrorResponse): void {
    ...
    if (!navigator.onLine && (error instanceof HttpErrorResponse || ErrorsHandler.chunkFailedMatch.test(error.message))) {
      ...
      this.notifyError(ErrorsHandler.defaultErrorTitle, 'ERROR.ERROR_OFFLINE', 'offline');
      return;
    }
    if (error instanceof HttpErrorResponse) {
      ...
      switch (status) {
        case 403:
          this.notifyError(ErrorsHandler.defaultErrorTitle, 'ERROR.NOT_AUTHORIZED', error.status.toString());
          this.injector.get(Router).navigate(['/'], {replaceUrl: true});
          return;
        case 413:
          this.notifyError(ErrorsHandler.defaultErrorTitle, 'ERROR.DIMENSION_ERROR', error.status.toString());
          return;
        default:
          break;
      }
      ...
      switch (Math.floor(status / 100)) {
        ...
        case 4:
          if (error.error && error.error.message) {
            this.notifyError(ErrorsHandler.defaultErrorTitle, error.error.message, error.status.toString());
            return;
          }
          this.notifyError(ErrorsHandler.defaultErrorTitle, 'ERROR.SYSTEM_ERROR', error.status.toString());
          return;
        // 5XX family errors
        case 5:
          if (error.error && error.error.message) {
        ...
        // no errors
        default:
          this.notifyError(ErrorsHandler.defaultErrorTitle, 'ERROR.ERROR_UNRECOGNIZED', error.status.toString());
          return;
      }
    } else {
  ...
  private notifyError(title: string, message: string, key: string, args?: any[]): void {
    const options: NzNotificationDataOptions<any> = {nzPauseOnHover: true, nzKey: key };
